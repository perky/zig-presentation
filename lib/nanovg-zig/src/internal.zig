...
pub const Context = struct {
    ...
    pub fn currentFontFaceName(ctx: *Context) [:0]const u8 {
        const state = ctx.getState();
        const name: [*c]const u8 = c.fonsGetFontNameById(ctx.fs, state.font_id);
    ...
    pub fn drawGlyph(ctx: *Context, x: f32, y: f32, glyph_id: u32) void {
        const state = ctx.getState();
        ...
        const q = c.fonsGetGlyphQuad(ctx.fs, x, y, glyph_id);
        // Transform corners.
