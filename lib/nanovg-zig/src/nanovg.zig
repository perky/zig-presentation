...
pub fn createImageFile(self: Self, allocator: std.mem.Allocator, filename: []const u8, flags: ImageFlags) !Image {
    var file = try std.fs.cwd().openFile(filename, .{ .mode = .read_only });
    defer file.close();
    var image_data = try file.readToEndAlloc(allocator, std.math.maxInt(usize));
    return self.ctx.createImageMem(image_data, flags);
}
...
pub fn glyph(self: Self, x: f32, y: f32, glyph_id: u32) void {
    return self.ctx.drawGlyph(x, y, glyph_id);
}
